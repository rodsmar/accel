		...
		list_for_each_entry(attr, &rpd->dm_coa_req->attrs, entry) {
			if (attr->vendor && attr->vendor->id == VENDOR_Mikrotik && 
				...
				if (attr->val.integer > 0) {
					force_disconnect = 1;
					log_ppp_info1("radius: Disconnect-Request com Mikrotik-Force-Disconnect, desconectando cliente mesmo autenticado por Access-Reject\n");
					break;
				}
			}
		}
		
		// Se não tem o atributo de desconexão forçada e foi autenticado por Access-Reject, ignora
		if (!force_disconnect) {
			log_info1("radius: ignorando Disconnect-Request para sessão autenticada por Access-Reject\n");
			dm_coa_send_nak(serv.hnd.fd, rpd->dm_coa_req, &rpd->dm_coa_addr, 503); // Código de erro: sessão não pode ser finalizada
...
static void dm_coa_recv(int type, struct rad_packet_t *req, struct rad_packet_t *reply)
{
	struct dm_coa_ctx_t *ctx = NULL;
	struct rad_attr_t *attr;
	struct radius_pd_t *rpd = NULL;
	const char *sessionid = NULL;
	int res = 0;

	if (type == DM_COA_DISCONNECT_REQUEST) {
		// Verifica se a sessão foi autenticada por Access-Reject
		if (rpd->reject_authenticated) {
			log_info1("radius: sessão %s autenticada por Access-Reject, ignorando Disconnect-Request\n", sessionid);
			res = -1;  // Ignorar solicitação de desconexão
			goto out;
		}

		if (conf_require_nas_ident && !nas_match(req)) {
			log_warn("radius: NAS identification failed\n");
			res = -1;
			goto out;
		}
	}
