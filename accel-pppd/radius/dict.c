...
/* Predefined vendors */
static struct rad_vendor_t vendors[] = {
	{ .id = VENDOR_Mikrotik, .name = "Mikrotik" },
};

/* Mikrotik attributes */
static struct rad_dict_attr_t mikrotik_attrs[] = {
	{ .id = Mikrotik_Rate_Limit, .name = "Mikrotik-Rate-Limit", .type = ATTR_TYPE_STRING },
	{ .id = Mikrotik_Force_Disconnect, .name = "Mikrotik-Force-Disconnect", .type = ATTR_TYPE_INTEGER },
	{ .id = -1 }
};

static void load_vendors(void)
{
	struct rad_dict_vendor_t *v;
	struct rad_vendor_t *vendor;
	int i;

	for (i = 0; vendors[i].id; i++) {
		vendor = rad_dict_find_vendor_id(vendors[i].id);
		if (vendor)
			continue;
		rad_dict_add_vendor(vendors[i].id, vendors[i].name);
	}

	// Adicionar os atributos do Mikrotik
	vendor = rad_dict_find_vendor_name("Mikrotik");
	if (vendor) {
		for (i = 0; mikrotik_attrs[i].id != -1; i++)
			rad_dict_add_attr(vendor, mikrotik_attrs[i].id, mikrotik_attrs[i].name, mikrotik_attrs[i].type);
	}
}
