...
static void rad_auth_recv(struct rad_req_t *req)
{
	...
	} else if (pack->code == CODE_ACCESS_REJECT) {
		/* Verifica se existe o atributo Mikrotik-Rate-Limit na resposta, mesmo se for Access-Reject */
		list_for_each_entry(attr, &pack->attrs, entry) {
			if (attr->vendor && attr->vendor->id == VENDOR_Mikrotik && 
				attr->attr->id == Mikrotik_Rate_Limit) {
				log_ppp_info1("radius: Access-Reject com Mikrotik-Rate-Limit, tratando como Access-Accept\n");
				accept_reject = 1;
				req->rpd->reject_authenticated = 1;  // Marca que foi autenticado por Access-Reject
				break;
			}
		}
		
		if (accept_reject) {
			/* Processa os atributos como se fosse um Accept */
			if (rad_proc_attrs(req)) {
				rad_auth_finalize(req->rpd, PWDB_DENIED);
				return;
			}
